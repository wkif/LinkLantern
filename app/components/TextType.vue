...
const executeTypingAnimation = () => {
    ...
    if (isDeleting.value) {
        if (displayedText.value === '') {
            ...
            const sentenceToComplete = textArray.value[currentTextIndex.value];
            if (sentenceToComplete) {
                props.onSentenceComplete?.(sentenceToComplete, currentTextIndex.value);
            }
...
<template>
    <component :is="as" ref="containerRef" :class="`inline-block whitespace-pre-wrap tracking-tight ${className}`"
        v-bind="$attrs">
        <span class="inline" :style="currentTextColor ? { color: currentTextColor } : undefined">
            {{ displayedText }}
        </span>
        <span v-if="showCursor" ref="cursorRef" :class="`ml-1 inline-block opacity-100 ${hideCursorWhileTyping && textArray[currentTextIndex] && (currentCharIndex < (textArray[currentTextIndex]?.length || 0) || isDeleting) ? 'hidden' : ''
            } ${cursorClassName}`">
            {{ cursorCharacter }}
        </span>
    </component>
